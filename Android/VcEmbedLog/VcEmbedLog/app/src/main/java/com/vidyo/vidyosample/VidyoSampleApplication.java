...
public class VidyoSampleApplication extends android.app.Application {
	...
	private String getAndroidSDcardMemDir() throws IOException {
		...
		File dir = new File(sdCard.getAbsolutePath() + "/VidyoSample/");
		dir.mkdirs();
	...
	public void initializeLogFile() {
		try {
			if (canWriteOnExternalStorage() == true) {
				String folder = getAndroidSDcardMemDir();
				folderObject = new File(folder);
				folderObject.mkdirs();
				String logFileName = folder + "Sample.log";
	...
	public boolean initialize(String caFileName, String uniqueId, Activity activity) {
		...
		try {
			...
			String  logDir = getAndroidSDcardMemDir();
...
//			logDir = getAndroidSDcardMemDir();
	...
	public void onReceivedLogCallback(String logMessage) {
		if(logWriter == null)
			return;
		try {
