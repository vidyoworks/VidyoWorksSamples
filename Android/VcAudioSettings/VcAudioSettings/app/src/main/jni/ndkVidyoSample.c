				   ...
				   VidyoVoidPtr data)
{
	...
	else if(event == VIDYO_CLIENT_OUT_EVENT_MUTED_AUDIO_OUT)
	{
		...
		VidyoClientOutEventMuted *muted;
		muted = (VidyoClientOutEventMuted *) param;
		if(muted->isMuted == VIDYO_FALSE)
		{
			LOGW("VIDYO_CLIENT_OUT_EVENT_MUTED_AUDIO_OUT\n");
			SampleSpeakerMuted();
...
void SampleSpeakerMuted()
{
	...
	jmethodID mid;
	jstring js;
	LOGE("SampleSpeakerMuted Begin");
	env = getJniEnv(&isAttached);
	if (env == NULL)
		goto FAIL0;

	mid = getApplicationJniMethodId(env, applicationJniObj, "speakerMutedCallback", "()V");
	if (mid == NULL)
		goto FAIL1;
	...
	FAIL0:
	LOGE("SampleSpeakerMuted FAILED");
	return;
}
