...
void LibraryStarted();
void OnReceivedLog(char* logmsg);
				   ...
				   VidyoVoidPtr data)
{
	...
	if(event == VIDYO_CLIENT_OUT_EVENT_LOG)
	{
		VidyoClientOutEventLog* log = NULL;
		if(param) {
			VidyoClientOutEventLog *log = (VidyoClientOutEventLog *) param;
			LOGE("********=%s", log->message);
			OnReceivedLog(log->message);
			LOGE("********LOG EVENT COMPLETE ********************");
		}
		else
			LOGE("********null log param ********************");
	}
	else if(event == VIDYO_CLIENT_OUT_EVENT_LICENSE)
	{
...
void OnReceivedLog(char * log)
{
	...
	mid = getApplicationJniMethodId(env, applicationJniObj, "onReceivedLogCallback", "(Ljava/lang/String;)V");
	if (mid == NULL)
		goto FAIL1;

	js = (*env)->NewStringUTF(env, log);
	...
	return;
FAIL1:
		LOGE("Callback onReceivedLogCallback not found");
		if (isAttached)
		{
			(*global_vm)->DetachCurrentThread(global_vm);
		}
FAIL0:
		LOGE("OnReceivedLog Jni not loaded");
return;
}
