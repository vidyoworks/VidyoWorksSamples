...
jboolean gSelfViewWatched = JNI_FALSE;
jboolean IsSelfViewWatched();
void FrameReceived(int width, int height, const char *rawImageBytes, int size);
				   ...
				   VidyoVoidPtr data)
{
	...
	else if (event == VIDYO_CLIENT_OUT_EVENT_SELF_VIEW_VIDEO_FRAME_RECEIVED && IsSelfViewWatched())
	{
		...
		else if (format == VIDYO_CLIENT_MEDIA_CONTROL_TYPE_VIDEO)
		{
			...
			FrameReceived(width, height, event->frame.data, event->frame.size);
		}
		else
			LOGE("-----------  strange path");
...
JNIEXPORT void JNICALL Java_com_vidyo_vidyosample_VidyoSampleApplication_MuteRendering(JNIEnv *env, jobject jobj, jboolean mute)
{
	VidyoClientInEventMute event;
	LOGI("***** **** ***** MUTE VAL =%d \n", mute);

	if(mute){
		event.willMute = VIDYO_TRUE;
		LOGI("**** RENDERING MUTE");
	}
	else {
		event.willMute = VIDYO_FALSE;
		LOGI("**** RENDERING UNMUTE");
	}
	VidyoClientSendEvent(VIDYO_CLIENT_IN_EVENT_MUTE_DEFAULT_RENDERING, &event, sizeof(VidyoClientInEventMute));

	if(mute){
		SetWatchFormat();
		gSelfViewWatched = JNI_TRUE;
		VidyoClientSendEvent(VIDYO_CLIENT_IN_EVENT_START_WATCH_SELF_VIEW, NULL, 0);


	}
	else {
		gSelfViewWatched = JNI_FALSE;
		VidyoClientSendEvent(VIDYO_CLIENT_IN_EVENT_STOP_WATCH_SELF_VIEW, NULL, 0);
	}
}

jboolean IsSelfViewWatched()
{
	return gSelfViewWatched;
}
...
void FrameReceived(int width, int height, const char *rawImageBytes, int size)
{
	...
	(*env)->CallVoidMethod(env, applicationJniObj, mid, 0, jWidth, jHeight, jRawImageBytes);
