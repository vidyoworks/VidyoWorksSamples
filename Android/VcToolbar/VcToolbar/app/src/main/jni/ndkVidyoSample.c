...
void CallMuteCallback(int muteType, jboolean muteValue)
{
    jboolean isAttached;
    JNIEnv *env;
    jmethodID mid;
    jstring js;
    LOGE("xxxxx CallMuteCallback Begin");
    env = (JNIEnv*)getJniEnv(&isAttached);
    if (env == NULL)
        goto FAIL0;
    mid = (jmethodID)getApplicationJniMethodId(env, applicationJniObj, "muteCallback", "(IZ)V");
    if (mid == NULL)
        goto FAIL1;

    (*env)->CallVoidMethod(env, applicationJniObj, mid, muteType, muteValue);

    if (isAttached)
    {
        (*global_vm)->DetachCurrentThread(global_vm);
    }
    LOGE("CallMuteCallback End");
    return;
    FAIL1:
    if (isAttached)
    {
				   ...
				   VidyoVoidPtr data)
{
    ...
    else if(event == VIDYO_CLIENT_OUT_EVENT_MUTED_VIDEO  || event == VIDYO_CLIENT_OUT_EVENT_MUTED_SERVER_VIDEO)
    {
        VidyoClientOutEventMuted *mute = (VidyoClientOutEventMuted *)param;
        LOGI("xxxxx VIDYO_CLIENT_OUT_EVENT_MUTED_VIDEO\n");
        CallMuteCallback(0, (jboolean)(mute->isMuted == VIDYO_TRUE));
    }
    else if(event == VIDYO_CLIENT_OUT_EVENT_MUTED_AUDIO_IN  || event == VIDYO_CLIENT_OUT_EVENT_MUTED_SERVER_AUDIO_IN)
    {
        VidyoClientOutEventMuted *mute = (VidyoClientOutEventMuted *)param;
        LOGI("xxxxx VIDYO_CLIENT_OUT_EVENT_MUTED_AUDIO_IN\n");
        CallMuteCallback(1, (jboolean)(mute->isMuted == VIDYO_TRUE));
    }
}
...
*/
    ...
    logParam.logLevelsAndCategories = "fatal error warning debug@App debug@AppGui info@AppGui";
...
//    logParam.pathToDumpDir = "/data/data/com.vidyo.vidyosample/files/";
    ...
    (void)VidyoClientSendEvent(VIDYO_CLIENT_IN_EVENT_ENABLE_BUTTON_BAR,
                             &event, sizeof(VidyoClientInEventEnable));

	AppCertificateStoreInitialize(logDirC,certificatesFileNameC,NULL);
    char test[]="fatal error warning debug@App info@AppEmcpClient debug@LmiApp debug@AppGui info@AppGui";
...
JNIEXPORT void JNICALL Java_com_vidyo_vidyosample_VidyoSampleApplication_ToolbarAction(JNIEnv *env, jobject jobj, jint actionType, jboolean muteValue)
{
	VidyoClientInEventMute mute;
	LOGI("xxxxx Java_com_vidyo_vidyosample_VidyoSampleApplication_ToolbarAction() enter\n");

	switch(actionType)
	{
		case 0:
            LOGI("xxxxx Camera mute\n");
			mute.willMute = muteValue;
			VidyoClientSendEvent(VIDYO_CLIENT_IN_EVENT_MUTE_VIDEO, &mute, sizeof(VidyoClientInEventMute));
			break;

		case 1:
            LOGI("xxxxx Mic mute\n");
			mute.willMute = muteValue;
			VidyoClientSendEvent(VIDYO_CLIENT_IN_EVENT_MUTE_AUDIO_IN, &mute, sizeof(VidyoClientInEventMute));
			break;
	...
	LOGI("xxxxx Java_com_vidyo_vidyosample_VidyoSampleApplication_ToolbarAction() exit\n");
}
