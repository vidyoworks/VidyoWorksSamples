...
JNIEXPORT void Java_com_vidyo_vidyosample_VidyoSampleApplication_SetBackgroundColor(JNIEnv* env, jobject javaThis, jint value) {

	FUNCTION_ENTRY;
	LOGI("Java_com_vidyo_vidyosample_VidyoSampleApplication_SetBackgroundColor() enter\n");
	...
	VidyoClientSendEvent(VIDYO_CLIENT_IN_EVENT_SET_BACKGROUND_COLOR, &event, sizeof(VidyoClientInEventColor));
	FUNCTION_EXIT;
}

JNIEXPORT void Java_com_vidyo_vidyosample_VidyoSampleApplication_SetActiveInterface(JNIEnv* env, jobject javaThis, jstring name) {

	FUNCTION_ENTRY;
	LOGI("Java_com_vidyo_vidyosample_VidyoSampleApplication_SetActiveInterface() enter\n");

	const char *cname = (*env)->GetStringUTFChars(env, name, NULL);

	VidyoClientInEventSetNetworkInterface event = {0};


	strlcpy(event.name, cname, sizeof(event.name));

	VidyoClientSendEvent(VIDYO_CLIENT_IN_EVENT_SET_NETWORK_INTERFACE, &event, sizeof(VidyoClientInEventSetNetworkInterface));
	FUNCTION_EXIT;
}
