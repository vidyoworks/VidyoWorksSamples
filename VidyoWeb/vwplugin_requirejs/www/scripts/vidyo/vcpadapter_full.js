        ...
        var locVcpadapter = {
           ...
           detectPlugin: function() {
              ...
              if (this.isChrome) {
                 ...
                 // *** !!! ***
                 logger.log('error', 'vcpadapter', "unexpected execution path in detectPlugin()");
           ...
           startVidyoClient : function(contextObject) {
              ...
              if (proxyWrapper.isChrome) { //checks to see if is chrome, if chrome will start vidyoforWeb, if not the npapi plugin
                 ...
                 // *** !!! ***
                 logger.log('error', 'vcpadapter', "unexpected execution path in startVidyoClient()");
                 return(false);
                 // *** !!! ***
          ...
          detectVidyoClientForWeb: function() { //method to check for vidyo client for web
            ...
            // *** !!! ***
            logger.log('error', 'vcpadapter', "unexpected execution path in detectVidyoClientForWeb()");
            return;
            // *** !!! ***
          ...
          detectChromePlugin: function() { //will try to search for chrome plugin
            ...
            // *** !!! ***
            logger.log('error', 'vcpadapter', "unexpected invocation of detectChromePlugin()");
            return;
            // *** !!! ***
          ...
          addPluginToDom : function() {
            ...
            if (!pluginHTML) { // it's not added, so will try to add
                logger.log('info', 'vcpadapter', "Attempting to add plugin to DOM ...");
                ...
                try {
                    ...
                    logger.log('error', 'vcpadapter', mesg);
            ...
            logger.log('info', 'vcpadapter', "succesfully loaded the plugin");
          ...
          startVidyoClientForWeb : function() { //will init vidyoForWeb
            ...
            // *** !!! ***
            logger.log('error', 'vcpadapter', "unexpected invocation of startVidyoClientForWeb()");
            return;
            // *** !!! ***
          ...
          startPlugin : function (contextObject)   {
              ...
              var locConfig = {
                  ...
                  defaultOutEventCallbackMethod:              // M.F. --- should NOT get called
                     function (event) {
                        logger.log('warning', 'vcpadapter', "invoked default callback for VidyoClient event: ",
                           event.type);
                     },
              ...
              if (contextObject.client.start(locConfig)) {
                  logger.log('info', 'vcpadapter', "VidyoClient lib started successfully");
                  retv = true;
              } else {
                  logger.log('error', 'vcpadapter', "failed to start VidyoClient lib!");
                  retv = false;
              }
          return(retv);
          },
