...
static int snReceivedFromVc = 0;
static int snSentToUi = 0;
              ...
              VidyoVoidPtr data)
{
        ...
        && event <= VIDYO_CLIENT_OUT_EVENT_MAX))
    {
        if(paramSize)
        {
            copyParam = malloc(paramSize);
            if(!copyParam)
                return;
            memcpy(copyParam, param, paramSize);
            if(event == VIDYO_CLIENT_OUT_EVENT_VIDEO_FRAME_RECEIVED)
            {
                snReceivedFromVc = (++snReceivedFromVc) % 30000;
                qDebug() << ">>>>>>>> from Vc -  " << snReceivedFromVc;
...
#ifdef Q_WS_WIN
    static const char folder[] = {"logs\\"};
#else
    //QString folder = QDir::homePath()+ QString("/VsLog/");
    QString folder = QString("/Users/dinesh/VsLog/");
#endif

    logParams.pathToLogDir = folder;
    logParams.pathToDumpDir = folder;
    logParams.pathToConfigDir = folder;
    ...
    switch (event)
        {
            ...
            case VIDYO_CLIENT_OUT_EVENT_MUTED_VIDEO:
             ...
             case VIDYO_CLIENT_OUT_EVENT_VIDEO_FRAME_RECEIVED:
                {
                snSentToUi = (++snSentToUi) % 30000;
                qDebug() << ".............. to ui -  " << snSentToUi;
