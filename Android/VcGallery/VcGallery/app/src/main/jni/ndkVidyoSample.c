...
void SetNumberOfSources(int number)
{
	VidyoClientInEventParticipantsLimit event;
	event.maxNumParticipants = number;
	VidyoClientSendEvent(VIDYO_CLIENT_IN_EVENT_PARTICIPANTS_LIMIT, &event, sizeof(VidyoClientInEventParticipantsLimit));
}


JNIEXPORT void JNICALL Java_com_vidyo_vidyosample_VidyoSampleApplication_SetNumberOfSources(JNIEnv *env, jobject jobj, jint sources)
{
	LOGI("Java_com_vidyo_vidyosample_VidyoSampleApplication_SetNumberOfSources enter\n");
	SetNumberOfSources((int)sources);
	LOGI("Java_com_vidyo_vidyosample_VidyoSampleApplication_SetNumberOfSources exit\n");
}


JNIEXPORT void JNICALL Java_com_vidyo_vidyosample_VidyoSampleApplication_PinSource(JNIEnv *env, jobject jobj, jint pinSourceIndex)
{
	LOGI("Java_com_vidyo_vidyosample_VidyoSampleApplication_PinSource() enter\n");


	VidyoClientRequestParticipants reqParticipants;
	VidyoClientInEventSetParticipantVideoMode pinRequest;
	int nPinSourceIndex = (int)pinSourceIndex;

	LOGI("Find the number of participants");

	VidyoClientSendRequest(VIDYO_CLIENT_REQUEST_GET_PARTICIPANTS, &reqParticipants, sizeof(reqParticipants));
	if(nPinSourceIndex >= reqParticipants.numberParticipants)
	{
		LOGI("Index to pin is more than the number of participants");
		return;
	}
	...
	LOGI("Going to pin now");
	strcpy(pinRequest.uri,reqParticipants.URI[nPinSourceIndex]);
	...
	LOGI("Java_com_vidyo_vidyosample_VidyoSampleApplication_PinSource() exit\n");
}
