        ...
        break;
            ...
            case VIDYO_CLIENT_OUT_EVENT_MUTED_VIDEO:
             ...
             case VIDYO_CLIENT_OUT_EVENT_VIDEO_FRAME_RECEIVED:
                ...
                if(!m_monitoredUri.isEmpty())
                {
                    VidyoClientInEventVideoFrame* srcVideoFrame = &videoFrame->frame;
                    QString uri = QString::fromUtf8((const char *)(videoFrame->participantURI));
                    if(m_monitoredUri.compare(uri) == 0)
                    {
                        int sizeOfBuffer = sizeof(VidyoClientInEventVideoFrame) + srcVideoFrame->size;
                        VidyoClientInEventVideoFrame* inVideoFrame = (VidyoClientInEventVideoFrame*)malloc(sizeOfBuffer);
                        memcpy(inVideoFrame,srcVideoFrame, sizeOfBuffer);

                        VidyoClientSendEvent(VIDYO_CLIENT_IN_EVENT_SEND_VIDEO_FRAME, inVideoFrame, sizeOfBuffer);
                        free(inVideoFrame);
                    }
                }
                return;
...
void VsPlugin::StartCapture(const QString& uri)
{
    m_monitoredUri = uri;
}
