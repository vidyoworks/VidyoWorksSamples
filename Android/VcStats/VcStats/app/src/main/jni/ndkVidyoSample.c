...
//    logParam.pathToDumpDir = "/data/data/com.vidyo.vidyosample/files/";
    ...
    char test[]="fatal error warning debug@App info@AppEmcpClient debug@LmiApp debug@AppGui info@AppGui debug@LmiUi all@LmiIce all@LmiStun";
...
JNIEXPORT void JNICALL Java_com_vidyo_vidyosample_VidyoSampleApplication_ShowStats(JNIEnv *env, jobject javaThisj)
{
	FUNCTION_ENTRY
	req.visibility = VIDYO_TRUE;
	SendPrivateRequest(VIDYO_CLIENT_PRIVATE_REQUEST_MOBILE_SET_STATS_VISIBILITY, &req, sizeof(VidyoClientPrivateRequestVisibility), 0);
	FUNCTION_EXIT;
}
