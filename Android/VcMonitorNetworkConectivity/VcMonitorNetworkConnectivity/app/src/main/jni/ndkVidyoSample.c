...
void SampleIsPortalAvailable(jboolean value);
void LibraryStarted();
				   ...
				   VidyoVoidPtr data)
{
    ...
    else if(event == VIDYO_CLIENT_OUT_EVENT_IS_PORTAL_AVAILABLE)
    {
		LOGI("......... VIDYO_CLIENT_OUT_EVENT_IS_PORTAL_AVAILABLE\n");
        VidyoClientOutEventIsPortalAvailable * portalAvail = (VidyoClientOutEventIsPortalAvailable*)param;
        SampleIsPortalAvailable((jboolean)portalAvail->isPortalAvailable == VIDYO_TRUE);
    }
...
void SampleIsPortalAvailable(jboolean value)
{
    ...
    jmethodID mid;
    jstring js;
    LOGE("SampleIsPortalAvailable Begin");
    env = getJniEnv(&isAttached);
    if (env == NULL)
        goto FAIL0;
	LOGE("SampleIsPortalAvailable found %d", value);

    
    mid = getApplicationJniMethodId(env, applicationJniObj, "OnPortalAvailable", "(Z)V");
    if (mid == NULL)
        goto FAIL1;
    (*env)->CallVoidMethod(env, applicationJniObj, mid, value);
    
    if (isAttached)
    {
        (*global_vm)->DetachCurrentThread(global_vm);
    }
    LOGE("SampleIsPortalAvailable End");
    return;
FAIL1:
    if (isAttached)
    {
...
FAIL0:
    LOGE("SampleIsPortalAvailable FAILED");
    return;
}
...
JNIEXPORT jboolean JNICALL Java_com_vidyo_vidyosample_VidyoSampleApplication_IsLinkAvailable(JNIEnv *env, jobject javaThisj)
{
...
void JNICALL Java_com_vidyo_vidyosample_VidyoSampleApplication_IsPortalAccessible(JNIEnv *env, jobject javaThisj, jstring portal)
{
    FUNCTION_ENTRY;
    LOGI("Java_com_vidyo_vidyosample_VidyoSampleApplication_IsPortalAccessible() enter\n");
    
    const char *portalC = (*env)->GetStringUTFChars(env, portal, NULL);
    
   VidyoClientInEventIsPortalAvailable reqIsPortalAvail = {};
    
    strlcpy(reqIsPortalAvail.portalUri, portalC, sizeof(reqIsPortalAvail.portalUri));
    LOGI("Portal to check = %s\n", reqIsPortalAvail.portalUri);
    
    VidyoClientSendEvent(VIDYO_CLIENT_IN_EVENT_IS_PORTAL_AVAILABLE,
                         &reqIsPortalAvail,
